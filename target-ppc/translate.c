...
#define qemu_log_enabled() (0)
#define log_cpu_state_mask(...)
...
void ppc_translate_init(void)
{
    ...
    for (i = 0; i < 32; i++) {
        sprintf(p, "r%d", i);
        cpu_gpr[i] = tcg_global_mem_new(TCG_TYPE_I32, TCG_AREG0,
    ...
    cpu_nip = tcg_global_mem_new(TCG_TYPE_I32, TCG_AREG0,
                                 offsetof(CPUState, nip), "nip");

    cpu_msr = tcg_global_mem_new(TCG_TYPE_I32, TCG_AREG0,
                                 offsetof(CPUState, msr), "msr");

    cpu_ctr = tcg_global_mem_new(TCG_TYPE_I32, TCG_AREG0,
                                 offsetof(CPUState, ctr), "ctr");

    cpu_lr = tcg_global_mem_new(TCG_TYPE_I32, TCG_AREG0,
                                offsetof(CPUState, lr), "lr");

    cpu_xer = tcg_global_mem_new(TCG_TYPE_I32, TCG_AREG0,
                                 offsetof(CPUState, xer), "xer");

    cpu_reserve = tcg_global_mem_new(TCG_TYPE_I32, TCG_AREG0,
                                           ...
                                           int add_ca, int compute_ca, int compute_ov)
{
    ...
    if (add_ca) {
        t1 = tcg_temp_local_new(TCG_TYPE_I32);
...
GEN_HANDLER(subfic, 0x08, 0xFF, 0xFF, 0x00000000, PPC_INTEGER)
{
    ...
    tcg_gen_andi_tl(cpu_xer, cpu_xer, ~(1 << XER_CA));
    TCGv t0 = tcg_temp_local_new(TCG_TYPE_I32);
    TCGv t1 = tcg_const_tl(SIMM(ctx->opcode));
...
GEN_HANDLER(dozo, 0x1F, 0x08, 0x18, 0x00000000, PPC_POWER_BR)
{
    ...
    TCGv t0 = tcg_temp_new(TCG_TYPE_I32);
    TCGv t1 = tcg_temp_new(TCG_TYPE_I32);
    TCGv t2 = tcg_temp_new(TCG_TYPE_I32);
    /* Start with XER OV disabled, the most likely case */
    tcg_gen_andi_tl(cpu_xer, cpu_xer, ~(1 << XER_OV));
    tcg_gen_brcond_tl(TCG_COND_GE, cpu_gpr[rB(ctx->opcode)], cpu_gpr[rA(ctx->opcode)], l1);
...
GEN_HANDLER(maskg, 0x1F, 0x1D, 0x00, 0x00000000, PPC_POWER_BR)
{
    ...
    TCGv t1 = tcg_temp_new(TCG_TYPE_I32);
    TCGv t2 = tcg_temp_new(TCG_TYPE_I32);
    TCGv t3 = tcg_temp_new(TCG_TYPE_I32);
