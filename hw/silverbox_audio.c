...
/* update this whenever you change the silverbox_audio_state structure */
#define  AUDIO_STATE_SAVE_VERSION  1

#define  QFIELD_STRUCT   struct silverbox_audio_state
...
{
    struct silverbox_audio_state*  s = opaque;
...
static void enable_audio(struct silverbox_audio_state *s, int enable)
{
...
#if USE_QEMU_AUDIO_IN
static void start_read(struct silverbox_audio_state *s, uint32_t count)
{
    //printf( "... silverbox audio start_read, count=%d\n", count );
    s->read_size = count;
...
static void start_read(struct silverbox_audio_state *s, uint32_t count)
{
    ...
    if (s->input_fd < 0) {
        ...
        if (s->input_fd < 0) {
            fprintf(stderr, "silverbox_audio could not open %s for audio input\n", s->input_source);
        ...
        if (s->input_is_wav) {
            if (read(s->input_fd, wav_header, sizeof(wav_header)) != sizeof(wav_header)) {
                fprintf(stderr, "silverbox_audio could not read WAV file header %s\n", s->input_source);
                s->input_fd = -1;
                s->input_source = NULL; // set to to avoid endless retries
                return;
            }
...
static uint32_t silverbox_audio_read(void *opaque, target_phys_addr_t offset)
{
    uint32_t ret;
    struct silverbox_audio_state *s = opaque;
    offset -= s->base;
	...
	case AUDIO_READ_BUFFER_AVAILABLE:
        ...
        default:
            cpu_abort (cpu_single_env, "silverbox_audio_read: Bad offset %x\n", offset);
            return 0;
    }
}

static void silverbox_audio_write(void *opaque, target_phys_addr_t offset, uint32_t val)
{
    struct silverbox_audio_state *s = opaque;
    offset -= s->base;
	...
	case AUDIO_SET_READ_BUFFER:
        ...
        default:
            cpu_abort (cpu_single_env, "silverbox_audio_write: Bad offset %x\n", offset);
    }
}

static void silverbox_audio_callback(void *opaque, int free)
{
    struct silverbox_audio_state *s = opaque;
    int new_status = 0;
...
#if USE_QEMU_AUDIO_IN
static void
silverbox_audio_in_callback(void *opaque, int avail)
{
    struct silverbox_audio_state *s = opaque;
    int new_status = 0;
...
static CPUReadMemoryFunc *silverbox_audio_readfn[] = {
   silverbox_audio_read,
   silverbox_audio_read,
   silverbox_audio_read
};

static CPUWriteMemoryFunc *silverbox_audio_writefn[] = {
...
void silverbox_audio_init(uint32_t base, qemu_irq irq, const char* input_source)
{
    ...
    s = (struct silverbox_audio_state *)qemu_mallocz(sizeof(*s));
    s->irq = irq;
    s->base = base;
    ...
    iomemtype = cpu_register_io_memory(0, silverbox_audio_readfn, silverbox_audio_writefn, s);
