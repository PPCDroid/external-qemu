...
#define  SILVERBOX_MMC_SAVE_VERSION  1
#define  QFIELD_STRUCT  struct silverbox_mmc_state
QFIELD_BEGIN(silverbox_mmc_fields)
...
static void  silverbox_mmc_save(QEMUFile*  f, void*  opaque)
{
    struct silverbox_mmc_state*  s = opaque;

    qemu_put_be32(f, s->buffer - phys_ram_base);
    qemu_put_struct(f, silverbox_mmc_fields, s);
}

static int  silverbox_mmc_load(QEMUFile*  f, void*  opaque, int  version_id)
{
    struct silverbox_mmc_state*  s = opaque;

    if (version_id != SILVERBOX_MMC_SAVE_VERSION)
        return -1;

    s->buffer = qemu_get_be32(f) + phys_ram_base;
    return qemu_get_struct(f, silverbox_mmc_fields, s);
}
...
static void silverbox_mmc_do_command(struct silverbox_mmc_state *s, uint32_t cmd, uint32_t arg)
{
    int result;
    int new_status = MMC_STAT_END_OF_CMD;
    int opcode = cmd & 63;

// fprintf(stderr, "silverbox_mmc_do_command opcode: %s (0x%04X), arg: %d\n", get_command_name(opcode), cmd, arg);
...
#define SET_R1_CURRENT_STATE(s)    ((s << 9) & 0x00001E00) /* sx, b (4 bits) */
    ...
    switch (opcode) {
	    ...
	    if (s->bs == NULL) {
                    fprintf(stderr, "SD card is unattached; shouldn't happen\n");
                    abort();
	    }
...
static void silverbox_mmc_write(void *opaque, target_phys_addr_t offset, uint32_t val)
{
    ...
    switch(offset) {
        ...
        case MMC_SET_BUFFER:
            ...
            break;
        case MMC_CMD:
            silverbox_mmc_do_command(s, val, s->arg);
            break;
        case MMC_ARG:
            s->arg = val;
            break;
        ...
        default:
            cpu_abort (cpu_single_env, "silverbox_mmc_write: Bad offset %llx\n", offset);
    }
}

static CPUReadMemoryFunc *silverbox_mmc_readfn[] = {
   silverbox_mmc_read,
   silverbox_mmc_read,
   silverbox_mmc_read
};

static CPUWriteMemoryFunc *silverbox_mmc_writefn[] = {
   silverbox_mmc_write,
   silverbox_mmc_write,
   silverbox_mmc_write
};

void silverbox_mmc_init(uint32_t base, qemu_irq irq, int id, BlockDriverState* bs)
{
    struct silverbox_mmc_state *s;
    int iomemtype;

    s = (struct silverbox_mmc_state *)qemu_mallocz(sizeof(*s));
    s->base = base;
    s->irq = irq;
    s->bs = bs;	/* can be NULL (then we report not card inserted) */

    iomemtype = cpu_register_io_memory(0, silverbox_mmc_readfn, silverbox_mmc_writefn, s);
    cpu_register_physical_memory(base, 0x400, iomemtype);

    register_savevm( "silverbox_mmc", 0, SILVERBOX_MMC_SAVE_VERSION,
                     silverbox_mmc_save, silverbox_mmc_load, s);
}
